...
class BaseController:
    ...
    def feedback_data(self):
        try:
            ...
            self.logger.debug(f"Received data: T={self.base_data.get('T')}, v={self.base_data.get('v')}")
            return self.base_data
...
# ESP32 Bridge - hardware interface with OLED service
class ESP32Bridge(Node):
    def __init__(self):
        ...
        self.get_logger().info('ESP32 Bridge started with OLED update service')

    # ========== OLED SERVICE HANDLER ==========
    
    def handle_oled_update(self, request, response):
        """Service callback to update OLED display line"""
        try:
            command = {
                "T": 3,
                "lineNum": request.line_num,
                "Text": request.text
            }
            self.base_controller.send_command(command)
            response.success = True
            response.message = f"Updated line {request.line_num}"
            self.get_logger().debug(f'OLED service: line {request.line_num} = "{request.text}"')
        except Exception as e:
            response.success = False
            response.message = f"Failed: {str(e)}"
            self.get_logger().error(f'OLED service error: {e}')
    ...
    def feedback_loop(self):
        ...
        if isinstance(self.base_controller.base_data, dict) and self.base_controller.base_data.get("T") == 1001:
